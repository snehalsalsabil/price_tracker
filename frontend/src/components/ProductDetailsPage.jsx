...
const ProductDetailsPage = ({ product }) => {
  // Destructuring product object to extract needed properties
  const {
    name,
    url: productUrl,
    img,
  ...
  // Format and reverse the dates for the price history chart
  const dates = priceHistory
    .map((history) => formatDate(new Date(history.date)))
    .reverse();
  // Reverse the prices for the price history chart
  const prices = priceHistory.map((history) => history.price).reverse();

  // Chart data configuration for ApexCharts
  const chartData = {
    options: {
      chart: {
        id: "price-chart",
    ...
    series: [
      {
        name: "Price",
        data: prices, // Price data for the chart
      },
    ],
  };
