...
# Constants for URLs and their respective metadata for scraping
...
# List of available URLs for scraping
available_urls = URLS.keys()


# Function to load authentication credentials from a JSON file
def load_auth():
...
# Load credentials for Bright Data from auth.json file
cred = load_auth()
auth = f'{cred["username"]}:{cred["password"]}'
browser_url = f'wss://{auth}@{cred["host"]}'


# Asynchronous function to perform a search on the given page using metadata
async def search(metadata, page, search_text):
    ...
    # Wait for the page to load completely after performing the search
    await page.wait_for_load_state()
    return page


# Asynchronous function to retrieve product information from the page
async def get_products(page, search_text, selector, get_product):
    ...
    async with asyncio.TaskGroup() as tg:
        for div in product_divs:
            # Asynchronous task to process each product division
            async def task(p_div):
                product = await get_product(p_div)

                # Skip products with no price or URL
                if not product["price"] or not product["url"]:
                    return

                # Check if all search words are in the product name
                for word in words:
                    if (
                        not product["name"]
...
# Function to post results to a specified endpoint
...
# Main asynchronous function to perform the scraping process
async def main(url, search_text, response_route):
    ...
    async with async_playwright() as pw:
        ...
        # Select the appropriate function for getting product information based on the URL
        def func(x):
            return None
        ...
        # Retrieve products and post results
